...
+ (NSString*)stringFromDistance:(NSDate*)fromDate {
    NSInteger distance = [[NSDate date] timeIntervalSinceDate:fromDate];
    distance = distance / 60;
    if(distance == 0) distance = 1;
    
    if(distance < 60) {
        if(distance == 1) {
            return [NSString stringWithFormat:@"%ld minute ago", (long)distance];
        } else {
            return [NSString stringWithFormat:@"%ld minutes ago", (long)distance];
        }
    }
    
    distance = distance / 60; // hours
    if(distance < 24) {
        if(distance == 1) {
            return [NSString stringWithFormat:@"%ld hour ago", (long)distance];
        } else {
            return [NSString stringWithFormat:@"%ld hours ago", (long)distance];
        }
    ...
    if(distance < 30) {
        if(distance == 1) {
            return [NSString stringWithFormat:@"%ld day ago", (long)distance];
        } else {
            return [NSString stringWithFormat:@"%ld days ago", (long)distance];
        }
    }
    ...
    if(distance < 60 * 24 * 365) {
        if(distance == 1) {
            return [NSString stringWithFormat:@"%ld month ago", (long)distance];
        } else {
            return [NSString stringWithFormat:@"%ld months ago", (long)distance];
        }
    }
    
    distance = distance / 12;
    if(distance == 1) {
        return [NSString stringWithFormat:@"%ld year ago", (long)distance];
    } else {
        return [NSString stringWithFormat:@"%ld years ago", (long)distance];
    }
}
...
+ (NSDate *)dateFromString:(NSString *)string withTimeZone:(NSString*)timeZone
{
    return [NSDate dateFromString:string withFormat:[NSDate formatDbString] withTimeZone:timeZone];
}

+ (NSDate *)dateFromString:(NSString *)string withFormat:(NSString *)format withTimeZone:(NSString*)timeZone
{
...
+ (NSString *)stringFromDate:(NSDate *)date withFormat:(NSString *)format withTimeZone:(NSString *)timeZone {
	return [date stringWithFormat:format];
}

+ (NSString *)stringFromDate:(NSDate *)date withTimeZone:(NSString *)timeZone {
	return [date string];
}
